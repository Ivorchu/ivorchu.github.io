  ...
  return (
    <div className="flex flex-col rounded-lg border border-slate-800 p-5 transition-colors hover:border-slate-700">
      <div className="mb-0.5 flex items-start justify-between gap-2">
        <span className="font-mono text-xs text-slate-500">{item.context}</span>
        {item.link && (
          <a
            href={item.link}
            target="_blank"
      ...
      <h3 className="mb-0.5 font-semibold leading-snug text-slate-200">{item.title}</h3>
      <p className="mb-3 font-mono text-xs text-slate-600">{item.period}</p>
      <p className="mb-3 text-sm leading-relaxed text-slate-400">{item.description}</p>
